...
def get_vtk_array_type(numpy_array_type):
    ...
    _np_vtk = {np.character:vtkConstants.VTK_UNSIGNED_CHAR,
                np.uint8:vtkConstants.VTK_UNSIGNED_CHAR,
                np.uint16:vtkConstants.VTK_UNSIGNED_SHORT,
                np.uint32:vtkConstants.VTK_UNSIGNED_INT,
                ULONG_TYPE_CODE:vtkConstants.VTK_UNSIGNED_LONG,
                np.int8:vtkConstants.VTK_CHAR,
                np.int16:vtkConstants.VTK_SHORT,
                np.int32:vtkConstants.VTK_INT,
                LONG_TYPE_CODE:vtkConstants.VTK_LONG,
                np.float32:vtkConstants.VTK_FLOAT,
                np.float64:vtkConstants.VTK_DOUBLE,
                np.complex64:vtkConstants.VTK_FLOAT,
                np.complex128:vtkConstants.VTK_DOUBLE}
    try:
        return _np_vtk[numpy_array_type]
    except KeyError:
        for key in _np_vtk:
            if np.issubdtype(numpy_array_type, key):
                return _np_vtk[key]

def get_vtk_to_numpy_typemap():
    """Returns the VTK array type to numpy array type mapping."""
    _vtk_np = {vtkConstants.VTK_BIT:np.bool,
                vtkConstants.VTK_CHAR:np.int8,
                vtkConstants.VTK_UNSIGNED_CHAR:np.uint8,
                vtkConstants.VTK_SHORT:np.int16,
                vtkConstants.VTK_UNSIGNED_SHORT:np.uint16,
                vtkConstants.VTK_INT:np.int32,
                vtkConstants.VTK_UNSIGNED_INT:np.uint32,
                vtkConstants.VTK_LONG:LONG_TYPE_CODE,
                vtkConstants.VTK_UNSIGNED_LONG:ULONG_TYPE_CODE,
                vtkConstants.VTK_ID_TYPE:ID_TYPE_CODE,
                vtkConstants.VTK_FLOAT:np.float32,
                vtkConstants.VTK_DOUBLE:np.float64}
    return _vtk_np
...
def numpy_to_vtk(num_array, deep=0):
    ...
    assert not np.issubdtype(z.dtype, complex), \
    ...
    if np.issubdtype(z.dtype, arr_dtype):
        z_flat = np.ravel(z)
    else:
...
def vtk_image_to_numpy(vtk_image):
    """Converts a VTK image data to a numpy array.

    Given a subclass of vtkImageData, this function returns an
    appropriate numpy array containing the same data -- it actually
    points to the same data.  

    WARNING: This does not work for bit arrays.
             This function assumes that the image is 2D!
    ...
    - vtk_image : `vtkImageData`
      ...
      The VTK image data to be converted.

    """
    vtk_image_shape = vtk_image.GetDimensions()
    vtk_data = vtk_image.GetPointData().GetScalars()
    typ = vtk_data.GetDataType()
    assert typ in get_vtk_to_numpy_typemap().keys(), \
           "Unsupported array type %s"%typ
    assert typ != vtkConstants.VTK_BIT, 'Bit arrays are not supported.'

    shape = vtk_data.GetNumberOfTuples(), \
            vtk_data.GetNumberOfComponents()

    # Get the data via the buffer interface
    dtype = get_numpy_array_type(typ)
    result = np.frombuffer(vtk_data, dtype=dtype)
    if shape[1] == 1:
        shape = (shape[0], )
        result.shape = shape
    else:
    ...
    return result.reshape(vtk_image_shape[0:2])