...
namespace boost {
  ...
  protected:
    ...
    edge_descriptor edge_at(edges_size_type edge_index) const {
			assert(false); // will not work with my mods
    ...
    degree_size_type out_degree(vertex_descriptor vertex) const {
      ...
      degree_size_type out_edge_count = 1;
			for(std::size_t dimension_index = 0;
				dimension_index < Dimensions;
				++dimension_index)
				out_edge_count *= 3;   // 3-neighborhood in each dim

			static unsigned int penalty[] = {9, 6, 4};
      degree_size_type num_blocked_dim = 0;
			out_edge_count -= 1;     // no loops with self
      ...
      for (std::size_t dimension_index = 0;
        ...
        if ((vertex[dimension_index] == 0) ||
            (vertex[dimension_index] == (length(dimension_index) - 1))) {
					if(!wrapped(dimension_index)){
						out_edge_count -= penalty[num_blocked_dim];
						num_blocked_dim ++;
					}
        }
      }
    ...
    (vertex_descriptor vertex,
     degree_size_type out_edge_index) const {
			...
			signed char   min_idx[Dimensions];
			signed char   max_idx[Dimensions];
			signed char   rng_idx[Dimensions];
			std::fill(min_idx,min_idx+Dimensions,(signed char)-1);
			std::fill(max_idx,max_idx+Dimensions,(signed char) 1);

      std::size_t dimension_index = 0;
			// for each dimension, determine the minimal and maximal steps we can make
			for(dimension_index=0;dimension_index<Dimensions;++dimension_index){
				if (!wrapped(dimension_index) ){
					if (vertex[dimension_index] == 0)
						min_idx[dimension_index] = 0;
					if (vertex[dimension_index] == (length(dimension_index) - 1))
						max_idx[dimension_index] = 0;
				}
				rng_idx[dimension_index] = max_idx[dimension_index]-min_idx[dimension_index] + 1;
			}

			// determine the index of (0,0,0...,0) in the resulting block
      edges_size_type     zeroidx = 0;
			std::size_t   index_divider = 1;
			for(dimension_index=0;dimension_index<Dimensions;++dimension_index){
				zeroidx += min_idx[dimension_index]==0 ? 0 : index_divider;
				index_divider *= rng_idx[dimension_index];
			}

			// we do not want self-connections, so to find the index of the
			// [out_edge_index] out edge in the block described by [min_idx] and
			// [max_idx], we need to add one if it is larger than [zeroidx].
			if(out_edge_index>=zeroidx){
				out_edge_index ++;
				//std::cout<<"Adding to out_edge_index!"<<std::endl;
			}
			//std::cout<<"zeroidx: "<<zeroidx<<std::endl;
			//for(dimension_index=0;dimension_index<Dimensions;++dimension_index){
			//  std::cout<<"min_idx["<<dimension_index<<"]: "<<(int)min_idx[dimension_index]<<std::endl;
			//  std::cout<<"max_idx["<<dimension_index<<"]: "<<(int)max_idx[dimension_index]<<std::endl;
			//  std::cout<<"rng_idx["<<dimension_index<<"]: "<<(int)rng_idx[dimension_index]<<std::endl;
			//}

			// determine the offsets for out_edge_index
			index_divider = 1;
      array<vertices_size_type, Dimensions> other;
			unsigned int s_dimension_index; // signed!
			for(s_dimension_index=0;s_dimension_index<Dimensions;++s_dimension_index){
				//other[s_dimension_index] = vertex[s_dimension_index] + (out_edge_index / index_divider) % rng_idx[s_dimension_index];
				other[s_dimension_index] = vertex[s_dimension_index] + min_idx[s_dimension_index] + ((out_edge_index/index_divider) % rng_idx[s_dimension_index]);
				//other[s_dimension_index] = rng_idx[s_dimension_index];
				index_divider *= rng_idx[s_dimension_index];
			}
			return (std::make_pair(vertex, other));
    }
    ...
    edge (BOOST_GRID_GRAPH_TYPE_MEM vertex_descriptor source_vertex,
      ...
      std::pair<BOOST_GRID_GRAPH_TYPE_MEM edge_descriptor, bool> edge_exists =
        std::make_pair(std::make_pair(source_vertex, destination_vertex), false);
			...
			BOOST_GRID_GRAPH_TYPE_MEM vertices_size_type sum_dim_difference = 0;
			bool offending_dim_found = false;
      ...
      for (std::size_t dimension_index = 0;
        ...
        if (dim_difference > 0) {
					sum_dim_difference += dim_difference;
          ...
          if ((dim_difference == 1) ||
            ...
            offending_dim_found = true;
            break;
          }
        }

      } // for dimension_index
			if(!offending_dim_found && sum_dim_difference>0)
