...
class ICP
{
	...
	public:
		...
		ICP(int maxiter=1000, float lambda=2.f, int overlapiter=10, float trim_alpha=0.4, float trim_beta=1.0) 
			: mVerbose(false), mDeterminedScale(1), mLambda(lambda), mOverlapIterations(overlapiter), mTrimAlpha(trim_alpha), mTrimBeta(trim_beta),mMaxIters(maxiter)
		{ }
		...
		//
		/// Returns the determined scale
		inline TPrecision getScale()const{return mDeterminedScale;}

		/// Returns the error after matching
		inline TPrecision getMatchingError()const{return mFinalError;}

		inline const TTree& modelTree() const {return mModelTree;}
	...
	private:
		...
		TPrecision mDeterminedScale;       ///< the determined scale
...
ICP_FQCLASS::getTrimmingParams(Iter begin, int p, Accu acc){
	TPrecision alpha = mTrimAlpha, beta=mTrimBeta, e=0.0;
...
ICP_FQCLASS::match(Iter begin, Iter end){
	return match(begin, end, VectorTranslator(), VectorRotator());
