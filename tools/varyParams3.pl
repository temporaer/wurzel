...
around 'vals'  => sub {
	...
	die "Variable ", $self->name, " in section ", $self->section, " (",$self->file,") has no values!\n" unless scalar @new;
	return \@new;
};
sub lazy {
	my $self = shift();
...
sub run_cmd{
  my $self = shift;
	...
	my $time = localtime;
	$cmd =~ s/(?<!\\)\$\{vp_time\}/$time/g;
	$cmd =~ s/(?<!\\)\$\{vp_default_cfg\}/$self->global_cfg->default_cfg_file/ge;
	$cmd =~ s/(?<!\\)\$\{vp_default_section\}/$self->global_cfg->default_section/ge;
