...
FILE  = $(DATAPATH)/$(BASE)
WHAT ?= mass
all: $(FILE).sato $(FILE)-wgraph.ser  treeinfo

# calculate tubularness measure
$(FILE)-upsampled.dat $(FILE).sato: $(FILE).dat
	python -O main.py $(BASE)

# calculate distance map, predecessor map, serialized graph
$(FILE)-d_map.dat $(FILE)-p_map.dat $(FILE)-wgraph.ser: $(FILE).sato $(FILE)-upsampled.dat
